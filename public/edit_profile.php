...
if ($rawProfileImage !== '' && (str_starts_with($rawProfileImage, 'http://') || str_starts_with($rawProfileImage, 'https://'))) {
    $profileImageSrc = $rawProfileImage;
} elseif ($rawProfileImage === '') {
    $profileImageSrc = $appRoot . '/assets/image/logo/default.png';
} else {
    if (str_starts_with($rawProfileImage, '/')) {
        $profileImageSrc = $appRoot . $rawProfileImage;
    } else {
        $profileImageSrc = $appRoot . '/' . ltrim($rawProfileImage, '/');
    }
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['update_profile'])) {
    ...
    $errors = [];
    $new_phone_norm = str_replace([' ', '-', '(', ')'], '', $new_phone);
    $postcode_norm = trim((string)$postcode);

    if (!preg_match('/^\d{5}$/', $postcode_norm)) {
        temp("info", "Postcode must be exactly 5 digits.");
        redirect();
    }
    if (!preg_match('/^\+60\d{9,10}$/', $new_phone_norm) && !preg_match('/^60\d{9,10}$/', $new_phone_norm) && !preg_match('/^0\d{9,10}$/', $new_phone_norm)) {
